            ...
            if ($config->Database->type == 'SQ') {
                ...
                try {
                    ...
                    if (isset($data['sum'])) {
                        if ($index == "conn") {
                            if ($data['sum'] == 'volume') {
                                if (strpos($query, 'conn_all_security_tags') !== false)
                                    $query = str_replace('count(*)', "sum(dst_nbytes)", $query);
                                else
                                    $query = str_replace('count(*)', "sum(dst_nbytes + src_nbytes)", $query);
                            }
                            if ($data['sum'] == 'packet') {
                                if (strpos($query, 'conn_all_security_tags') !== false)
                                    $query = str_replace('count(*)', "sum(dst_npackets)", $query);
                                else
                                    $query = str_replace('count(*)', "sum(dst_npackets + src_npackets)", $query);
                            }
                        }
                        ...
                        if (is_array($data['where'])) {
                            foreach ($data['where'] as $k => $v) {
                                if (is_string($v)) {
                                    ...
                                    if ($k == 'dst_hostname' && $index == "alert") {
                                        $where[] = $k . '="' . $v . '" or alertinfo_match_dst_hostname' . '="' . $v . '"';
                                    } else
                                        $where[] = $k . '="' . $v . '"';
                                }
                                if (is_int($v)) {
