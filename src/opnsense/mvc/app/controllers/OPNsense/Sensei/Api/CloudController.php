            ...
            if (file_exists($sensei->cloudToken)) {
                ...
                if (empty($host_uuid)) {
                    ...
                    if ((int)$return == 0) {
                        ...
                        $sensei->logger('It could not take node uuid. Return code is ' . $return);
                        return ['successful' => false, 'message' => "Could not get node uuid." . implode(', ', $output)];
                    }
                }
                $curl = curl_init();
                ...
                $admin_email = (string) $sensei->getNodeByReference('general.CloudManagementAdmin');
                if (empty($admin_email)) {
                    if (file_exists($sensei::eastpect_config)) {
                        $config = new ConfigIni($sensei::eastpect_config);
                        $admin_email = $config->senpai['admin-email'];
                    }
                }
                if (empty($admin_email)) {
                    return ['successful' => false, 'message' => "Admin Email could not be found"];
                }
                $jwt = ["exp" => strtotime("+10 minute"), "user_id" => $admin_email, "node_uuid" => $host_uuid];
                ...
                if ($ret_http_code != 200 && $ret_http_code != 201) {
                    ...
                    return ['successful' => false, 'message' => "Failure unregistering this device from Cloud Portal. It could be that you've already unregistered this device before." . $message];
                }
                if (file_exists($sensei->cloudToken))
                    unlink($sensei->cloudToken);
