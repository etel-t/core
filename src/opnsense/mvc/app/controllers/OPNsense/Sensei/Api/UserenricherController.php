...
class UserenricherController extends Controller
{
    ...
    private function exists($rest_data, $groups)
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare('SELECT username,deleted  FROM users_cache WHERE ip_address=:ip_address and created>:curr_time order by created desc limit 1');
            //$stmt->bindValue(':username', $rest_data->username);
            $stmt->bindValue(':ip_address', $rest_data->ip);
            ...
            if ($row['deleted'] != 0 || $row['username'] != $rest_data->username)
                return 0;
            return 1;
            //return $row['total'];
        } catch (\Exception $e) {
