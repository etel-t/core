                ...
                foreach ($lines as $line) {
                    if (!empty($line)) {
                        ...
                        if (
                            ...
                            $level = 'Error';
                            switch (strtolower(substr($tmp['event']['prio'], 0, 3))) {
                                case 'cri':
                                    $level = 'Critical';
                                    break;
                                case 'err':
                            ...
                            //$response[] = ['id' => $hash, 'status' => 0, 'time' => $tmp['event']['time'], 'notice_name' => 'engine_notice', 'notice' => '<strong style="color:red;">' . $tmp['event']['prio'] . '</strong> ' . $tmp['event']['title'] . ': ' . $tmp['event']['msg']];
                            $response[] = ['id' => $hash, 'status' => 0, 'time' => $tmp['event']['time'], 'notice_name' => 'engine_notice', 'notice' => '<strong>' . $level . ':</strong> ' . $tmp['event']['title'] . ': ' . $tmp['event']['msg']];
                        }
    ...
    public function dbversionAction()
    {
        try {
            ...
            if (file_exists($sensei->dbServerConf))
                ...
                $updateServerUrl = $sensei->getUpdateServerUrl();
                $uri = $updateServerUrl . '/updates/db/1.12/version_history.json';
            }
            $curl = curl_init();
            curl_setopt($curl, CURLOPT_URL, $uri);
            curl_setopt($curl, CURLOPT_TIMEOUT, 20);
