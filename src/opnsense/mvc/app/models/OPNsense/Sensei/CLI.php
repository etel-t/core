...
use \OPNsense\Sensei\Api\CloudController;
...
function removeRegister($sensei)
{
    try {
        print "checking register to cloud management...";
        if (!file_exists($sensei->cloudToken)) {
            print "done.." . PHP_EOL;
            return;
        }
        $cloudController = new CloudController();
        $response = $cloudController->removeregisterAction();
        if (!$response['successful']) {
            print __METHOD__ . ' Not Success: ' . $response['message'] . PHP_EOL;
        } else {
            print " unregister is success done.." . PHP_EOL;
        }
    } catch (\Exception $e) {
        print __METHOD__ . ' Exception: ' . $e->getMessage() . PHP_EOL;
    }
