        ...
        try {
            ...
            if (isset($query['$match'])) {
                ...
                */
                ...
                if (isset($query['$match']['dst_hostname']) && $collection_name == 'alert_all') {
                    $v = $query['$match']['dst_hostname'];
                    unset($query['$match']['dst_hostname']);
                    $query['$match']['$or'] = [
                        [
                            'dst_hostname' => isset($v['$regex']) ? ['$regex' => new \MongoDB\BSON\Regex($v['$regex'], 'i')] : ['$regex' => new \MongoDB\BSON\Regex($v, 'i')]
                        ],
                        [
                            'alertinfo.match.dst_hostname' => isset($v['$regex']) ? ['$regex' => new \MongoDB\BSON\Regex($v['$regex'], 'i')] : ['$regex' => new \MongoDB\BSON\Regex($v, 'i')]
                        ]
                    ];
                }
                $aggrt[]['$match'] = ['$and' => [$query['$match']]];
                ...
                if (isset($query['find']) && is_array($query['find'])) {
                    foreach ($query['find'] as $k => $v) {
                        if ($k == 'dst_hostname' && $collection_name == 'alert_all') {
                            unset($query['find']['dst_hostname']);
                            $query['find']['$or'] = [
                                [
                                    'dst_hostname' => isset($v['$regex']) ? ['$regex' => new \MongoDB\BSON\Regex($v['$regex'], 'i')] : ['$regex' => new \MongoDB\BSON\Regex($v, 'i')]
                                ],
                ...
                $this->Collection = new \MongoDB\Collection($this->manager, 'sensei', $collection_name);
                file_put_contents(self::log_file, __METHOD__ . '::QUERY::' . var_export(['$and' => [$query['find']]], true), FILE_APPEND);
                $cursors[] = $this->Collection->find(['$and' => [$query['find']]], $options);
            ...
            if (file_exists(self::log_file))
                file_put_contents(self::log_file, __METHOD__ . '::Exception::' . $e->getMessage() . PHP_EOL, FILE_APPEND);
