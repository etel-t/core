...
def set_criteria(query, index, type):
    ...
    if index == 'conn' and CRITERIA in ['volume', 'packets'] and type not in ['custom']:
        query = query.replace('"___SUM_FIELD___"', '{"$add": ["$src_nbytes","$dst_nbytes"]}' if CRITERIA == 'volume' else '{"$add": ["$src_npackets","$dst_npackets"]}')
        query = query.replace('__PCOLUMN__', ',"dst_nbytes":"$dst_nbytes","src_nbytes":"$src_nbytes"' if CRITERIA == 'volume' else  ',"dst_npackets":"$dst_npackets","src_npackets":"$src_npackets"')    
       
    if index == 'http' and CRITERIA in ['volume', 'packets']:
        query = query.replace('___SUM_FIELD___', '$rsp_body_len')
    
    query = query.replace('"___SUM_FIELD___"', '1')    
    query = query.replace('__PCOLUMN__', '')    
